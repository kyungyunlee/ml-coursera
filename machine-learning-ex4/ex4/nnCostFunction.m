...
hidden = sigmoid(X1*Theta1');

hidden = [ones(size(hidden,1),1) hidden];

h_theta = sigmoid(hidden*Theta2');

y1 = eye(num_labels)(y,:);
 
temp = sum(sum(-1 * y1 .* log(h_theta) - (1-y1) .* log(1-h_theta), 2),1);
J = 1/m * temp;
th1 = Theta1;
th2 = Theta2;
th1(:, [1]) = [];
th2(:, [1]) = [];
reg = lambda/(2*m) * (sum(sum(th1 .* th1,2),1) + sum(sum(th2 .* th2,2),1));
J = J+reg;
...
for i=1:m
    ...
    y2= ([1:num_labels] == y(i))'; 
    delta_3 = a3 - y2; % 10 x 1
    delta_2 = (delta_3' * Theta2)' .* [1;sigmoidGradient(z2)]; % 26 x 1
